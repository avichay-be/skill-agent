...
class ProjectInfo(BaseModel):
    ...
    report_number: int = Field(
        ..., alias="reportNumber", description="Report sequence number (מספר דוח)"
    )
    ...
    developer_name: str = Field(
        ..., alias="developerName", description="Developer/sponsor name (היזם)"
    )
    developer_id: str = Field(
        ..., alias="developerId", description="Company registration number (ח.פ.)"
    )
    bank_account: Optional[str] = Field(
        None, alias="bankAccount", description="Escrow bank account number"
    )
    ...
    contractor_name: Optional[str] = Field(
        None, alias="contractorName", description="General contractor name"
    )
    ...
    permit_date: Optional[str] = Field(
        None, alias="permitDate", description="Building permit date DD/MM/YYYY"
    )
    insurance_end: Optional[str] = Field(
        None, alias="insuranceEnd", description="Insurance policy expiry DD/MM/YYYY"
    )
    insurance_value: Optional[int] = Field(
        None, alias="insuranceValue", description="Insurance coverage amount (NIS)"
    )
...
class SalesData(BaseModel):
    ...
    total_value: Optional[int] = Field(
        None, alias="totalValue", description="Total project value incl. inventory (אלפי ₪)"
    )
    sold_units: Optional[int] = Field(None, alias="soldUnits", description="Number of units sold")
    sold_value: Optional[int] = Field(
        None, alias="soldValue", description="Total value of sold units (אלפי ₪)"
    )
    ...
    inventory_units: Optional[int] = Field(
        None, alias="inventoryUnits", description="Unsold units in inventory"
    )
...
class CostData(BaseModel):
    ...
    land_budget: Optional[int] = Field(
        None, alias="landBudget", description="Land cost budget (אלפי ₪)"
    )
    ...
    soft_costs_paid: Optional[int] = Field(
        None, alias="softCostsPaid", description="Soft costs paid (אלפי ₪)"
    )
    ...
    total_budget: Optional[int] = Field(
        None, alias="totalBudget", description="Total project budget (אלפי ₪)"
    )
    total_paid: Optional[int] = Field(
        None, alias="totalPaid", description="Total paid to date (אלפי ₪)"
    )
...
class CreditData(BaseModel):
    ...
    credit_limit: Optional[int] = Field(
        None, alias="creditLimit", description="Approved credit facility (אלפי ₪)"
    )
    ...
    equity_required: Optional[int] = Field(
        None, alias="equityRequired", description="Required equity (אלפי ₪)"
    )
    equity_current: Optional[int] = Field(
        None, alias="equityCurrent", description="Current equity (אלפי ₪)"
    )
...
class SurplusData(BaseModel):
    ...
    profit_amount: Optional[int] = Field(
        None, alias="profitAmount", description="Profit amount (אלפי ₪)"
    )
    profit_pct: Optional[float] = Field(
        None, alias="profitPct", description="Profit as percentage of costs"
    )
...
class MonitoringReportResult(BaseModel):
    ...
    project_info: ProjectInfo = Field(
        ..., alias="projectInfo", description="Project identification and details"
    )
    sales_data: SalesData = Field(
        default_factory=SalesData,
        alias="salesData",
        description="Sales, inventory, and revenue data",
    )
    costs: CostData = Field(
        default_factory=CostData, description="Budget, paid, and remaining costs"
    )
    progress: ProgressData = Field(
        default_factory=ProgressData, description="Physical and financial progress"
    )
    credit_and_guarantees: CreditData = Field(
        default_factory=CreditData,
        alias="creditAndGuarantees",
        description="Credit, guarantees, and equity",
    )
    surplus_analysis: SurplusData = Field(
        default_factory=SurplusData,
        alias="surplusAnalysis",
        description="Surplus, extraction, and profitability",
    )
    notes: List[str] = Field(
        default_factory=list, description="Key notes and highlights from the report"
    )
    footnotes: List[Footnote] = Field(
        default_factory=list, description="Source citations for every data point"
    )
